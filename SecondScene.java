...
public class SecondScene extends Stage {
    ...
    public SecondScene() {
        ...
        setToggleGroup();
        chooseDifficultyBox.getChildren().addAll(rookieButton, beginnerButton, intermediateButton, advancedButton);
        setAllocateSkillPointsStyle();
        setSliders();
        setToggleLogic();
        setSlideLogic();
        setContinueButtonStyle();
        attributes.getChildren().addAll(sailorLabel, sailorSlide, barterLabel, barterSlide, engineerLabel, engineerSlide, fighterLabel, fighterSlide);
        layout3.getChildren().addAll(characterName);
        layout2.getChildren().addAll(titleLabel);
        layout2.setAlignment(Pos.CENTER);
        configScreen.getChildren().addAll(layout2, layout3, gameDifficulty, chooseDifficultyBox, allocateSkillPointsLabel, attributes, continueButton);
    ...
    private void setSlideLogic() {
        sailorSlide.valueProperty().addListener(observable -> {
            playerpoints[0] = sailorSlide.getValue() + barterSlide.getValue() + engineerSlide.getValue() + fighterSlide.getValue();
        });
    ...
    private void setContinueButtonStyle() {
        ...
        continueButton.setOnAction(ae -> {
            if (characterName.getText().equals("")) {
                ...
                errorAlert.setContentText("You do not have enough skill points! Please change the difficulty or allocate less skill points.");
                errorAlert.showAndWait();
            } else if (playerpoints[0] < playerDiff[0].getSkillPointAllocation()) {
                Alert errorAlert = new Alert(Alert.AlertType.ERROR);
                errorAlert.setHeaderText("Distribution error");
                errorAlert.setContentText("You haven't used all of your skill points! Please change the difficulty or allocate more skill points..");
                errorAlert.showAndWait();
            } else {
